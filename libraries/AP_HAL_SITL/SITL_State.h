...
class HALSITL::SITL_State {
    ...
    friend class HALSITL::ADCSource;
public:
...
private:
    ...
    /**
     * presumably representing raw A/D converter output
    **/
    uint16_t sonar_pin_value = 0U;    // pin 0
    uint16_t airspeed_pin_value = 0U; // pin 1
    uint16_t voltage_pin_value = 0U;  // pin 13
    uint16_t current_pin_value = 0U;  // pin 12
public:
...
*/
    void m_update_gps(SITL::sitl_fdm const & fdm, bool have_lock);

    void m_update_ins(SITL::sitl_fdm const & fdm);
/*
...
*/
    ...
    float _rand_float();
    Vector3f _rand_vec3f();
    void _fdm_input_step();

    void wait_clock(uint64_t wait_time_usec);

    // internal state
    enum vehicle_type _vehicle = ArduPlane;
    uint16_t _framerate = 50U;
    uint8_t _instance = 0U;
    uint16_t _base_port = 5760U;
    ...
    uint32_t last_store_time_mag = 0U;
    ...
    uint32_t last_store_time_wind = 0U;
    ...
    uint32_t last_store_time_baro = 0U;
