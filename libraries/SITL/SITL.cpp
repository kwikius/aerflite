...
void SITL::simstate_send(mavlink_channel_t chan)
{
    ...
    auto yaw = state.yawDeg;
    if (yaw > 180_deg) {
        yaw -= 360_deg;
    }

    mavlink_msg_simstate_send(chan,
      quan::angle::rad{state.rollDeg}.numeric_value(),
      quan::angle::rad{state.pitchDeg}.numeric_value(),
      quan::angle::rad{yaw}.numeric_value(),
      state.xAccel.numeric_value(),
      state.yAccel.numeric_value(),
      state.zAccel.numeric_value(),
      rad_per_s{state.rollRate}.numeric_value(),
      rad_per_s{state.pitchRate}.numeric_value(),
      rad_per_s{state.yawRate}.numeric_value(),
      state.latitude.numeric_value() * 1.0e7,
      state.longitude.numeric_value() * 1.0e7
    );
}
...
void SITL::Log_Write_SIMSTATE(DataFlash_Class *DataFlash)
{
    ...
    struct log_AHRS pkt = {
        ...
        roll    : (int16_t)(state.rollDeg.numeric_value()*100),
        pitch   : (int16_t)(state.pitchDeg.numeric_value()*100),
        yaw     : (uint16_t)(wrap_360_cd(yaw.numeric_value()*100)),
        alt     : (float)state.altitude_asl.numeric_value(),
        lat     : (int32_t)(state.latitude.numeric_value()*1.0e7),
        lng     : (int32_t)(state.longitude.numeric_value()*1.0e7)
    };
                              ...
                              double *p, double *q, double *r)
{
   // double phi, theta, phiDot, thetaDot, psiDot;
    ...
    double const phi = ToRad(rollDeg);
    double const theta = ToRad(pitchDeg);
    double const phiDot = ToRad(rollRate);
    double const thetaDot = ToRad(pitchRate);
    double const psiDot = ToRad(yawRate);
