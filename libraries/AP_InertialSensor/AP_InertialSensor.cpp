...
const AP_Param::GroupInfo AP_InertialSensor::var_info[] = {
    ...
    // @Units: rad/s
    // @User: Advanced
    AP_GROUPINFO("GYROFFS",     3, AP_InertialSensor, m_ins_state[0].gyro.offset,  0),
    ...
    // @Units: rad/s
    // @User: Advanced
    AP_GROUPINFO("GYR2OFFS",    7, AP_InertialSensor, m_ins_state[1].gyro.offset,   0),
    ...
    // @Units: rad/s
    // @User: Advanced
    AP_GROUPINFO("GYR3OFFS",   10, AP_InertialSensor, m_ins_state[2].gyro.offset,   0),
    ...
    // @Range: 0.8 1.2
    // @User: Advanced
    AP_GROUPINFO("ACCSCAL",     12, AP_InertialSensor, m_ins_state[0].accel.scale,  0),
    ...
    // @Range: -3.5 3.5
    // @User: Advanced
    AP_GROUPINFO("ACCOFFS",     13, AP_InertialSensor, m_ins_state[0].accel.offset, 0),
    ...
    // @Range: 0.8 1.2
    // @User: Advanced
    AP_GROUPINFO("ACC2SCAL",    14, AP_InertialSensor, m_ins_state[1].accel.scale,   0),
    ...
    // @Range: -3.5 3.5
    // @User: Advanced
    AP_GROUPINFO("ACC2OFFS",    15, AP_InertialSensor,m_ins_state[1].accel.offset,  0),
    ...
    // @Range: 0.8 1.2
    // @User: Advanced
    AP_GROUPINFO("ACC3SCAL",    16, AP_InertialSensor, m_ins_state[2].accel.scale,   0),
    ...
    // @Range: -3.5 3.5
    // @User: Advanced
    AP_GROUPINFO("ACC3OFFS",    17, AP_InertialSensor, m_ins_state[2].accel.offset,  0),
    ...
    // @User: Advanced
    AP_GROUPINFO("USE", 20, AP_InertialSensor, m_ins_state[0].use,  1),
    ...
    // @Values: 0:Disabled,1:Enabled
    // @User: Advanced
    AP_GROUPINFO("USE2", 21, AP_InertialSensor, m_ins_state[1].use,  1),
    ...
    // @Values: 0:Disabled,1:Enabled
    // @User: Advanced
    AP_GROUPINFO("USE3", 22, AP_InertialSensor, m_ins_state[2].use,  0),
...
uint8_t AP_InertialSensor::register_gyro(uint16_t raw_sample_rate_hz)
{
    ...
    m_ins_state[_gyro_count].gyro.raw_sample_rate = raw_sample_rate_hz;
    return _gyro_count++;
}
...
uint8_t AP_InertialSensor::register_accel(uint16_t raw_sample_rate_hz)
{
    ...
    m_ins_state[_accel_count].accel.raw_sample_rate = raw_sample_rate_hz;
    return _accel_count++;
}
...
AP_InertialSensor::init(Sample_rate sample_rate)
{
    ...
    for (uint8_t i=0; i<get_accel_count(); i++) {
        auto & scale = m_ins_state[i].accel.scale;
        if (scale.get().is_zero()) {
            scale.set(Vector3f(1,1,1));
        }
    }
                                        ...
                                        float &trim_pitch)
{
    ...
    */
    ...
    for (uint8_t k=0; k<num_accels; k++) {
        ...
        auto & accel_state = m_ins_state[k].accel;
        orig_offset[k] = accel_state.offset.get();
        orig_scale[k]  = accel_state.scale.get();

        // clear accelerometer offsets and scaling
        accel_state.offset = Vector3f(0,0,0);
        accel_state.scale = Vector3f(1,1,1);
    }
    ...
    for (uint8_t i=0; i<6; i++) {
        ...
        while (num_samples < 400/update_dt_milliseconds) {
            ...
            for (uint8_t k=0; k<num_accels; k++) {
                Vector3f samp;
                auto & accel_state = m_ins_state[k].accel;
                if(get_delta_velocity(k,samp) && accel_state.delta_velocity_dt > 0) {
                    samp /= accel_state.delta_velocity_dt;
                } else {
                    samp = get_accel(k);
                }
                samples[k][i] += samp;
    ...
    for (uint8_t k=0; k<num_accels; k++) {
        ...
        bool success = _calibrate_accel(samples[k],
                                        new_offsets[k],
                                        new_scaling[k],
                                        m_ins_state[k].accel.max_abs_offsets,
                                        saved_orientation);
    ...
    if (num_ok == num_accels) {
        ...
        for (uint8_t k=0; k<num_accels; k++) {
            ...
            auto & accel_state = m_ins_state[k].accel;
            accel_state.offset.set(new_offsets[k]);
            accel_state.scale.set(new_scaling[k]);
        }
        ...
        for (uint8_t k=num_accels; k<m_max_instances; k++) {
            // clear unused accelerometer's scaling and offsets
             auto & accel_state = m_ins_state[k].accel;
             accel_state.offset = Vector3f(0,0,0);
             accel_state.scale = Vector3f(0,0,0);
        }
        _save_parameters();
...
failed:
    ...
    for (uint8_t k=0; k<num_accels; k++) {
        auto & accel_state = m_ins_state[k].accel;
        accel_state.offset.set(orig_offset[k]);
        accel_state.scale.set(orig_scale[k]);
    }
    _board_orientation = saved_orientation;
    _calibrating = false;
    return false;
...
uint32_t AP_InertialSensor::get_accel_clip_count(uint8_t instance) const
{
   if (instance < get_accel_count()) {
      return m_ins_state[instance].accel.clip_count;
   }else{
      return 0;
   }
}
...
//    }
   ...
   // return (get_gyro_health(instance) && m_ins_state[instance].use);
...
{
    return (instance < m_max_instances) &&
            get_gyro_health(instance) &&
            m_ins_state[instance].use;
}
...
bool AP_InertialSensor::accel_calibrated_ok_all() const
{
    ...
    if (get_accel_count() < m_max_instances) {
        for (uint8_t i=get_accel_count(); i<m_max_instances; i++) {
            auto & accel_state = m_ins_state[i].accel;
            const Vector3f &scaling = accel_state.scale.get();
            bool have_scaling = (!is_zero(scaling.x) && !is_equal(scaling.x,1.0f)) ||
              (!is_zero(scaling.y) && !is_equal(scaling.y,1.0f)) || (!is_zero(scaling.z) && !is_equal(scaling.z,1.0f));
            bool have_offsets = accel_state.offset.get().is_zero();
...
bool AP_InertialSensor::use_accel(uint8_t instance) const
{
    ...
    return (get_accel_health(instance) && m_ins_state[instance].use);
}
...
AP_InertialSensor::_init_gyro()
{
    ...
    */
    ...
    for (uint8_t k=0; k<num_gyros; k++) {
        m_ins_state[k].gyro.offset.set(Vector3f());
    ...
    for (int16_t j = 0; j <= 30*4 && num_converged < num_gyros; j++) {
        // hal.console->print("###---###\n");
        Vector3f gyro_sum[m_max_instances];
        Vector3f gyro_avg[m_max_instances];
        Vector3f gyro_diff[m_max_instances];
...
// Quan wait for sample ?
   ...
   hal.console->println();
   for (uint8_t k=0; k<num_gyros; k++) {
      auto & gyro_state = m_ins_state[k].gyro;
      if (converged[k]){
         gyro_state.offset = new_gyro_offset[k];
         gyro_state.cal_ok = true;
      }else{
         hal.console->printf("gyro[%u] did not converge: diff=%f dps\n",
            (unsigned)k, (double)ToDeg(best_diff[k]));
         gyro_state.offset = best_avg[k];
         gyro_state.cal_ok = false;
      }
   }
   // restore orientation
   _board_orientation = saved_orientation;
                                         ...
                                         enum Rotation rotation)
{
    ...
    // sanity check scale
    if( accel_scale.is_nan() || fabsf(accel_scale.x-1.0f) > 0.1f || fabsf(accel_scale.y-1.0f) > 0.1f ||
       fabsf(accel_scale.z-1.0f) > 0.1f ) {
        success = false;
...
void AP_InertialSensor::_save_parameters()
{
    ...
    for (uint8_t i=0; i<m_max_instances; i++) {
        auto & ins_state = m_ins_state[i];
        ins_state.accel.scale.save();
        ins_state.accel.offset.save();
        ins_state.gyro.offset.save();
    }
}
...
void AP_InertialSensor::update(void)
{
    ...
    if (!_hil_mode) {
        for (uint8_t i=0; i<m_max_instances; i++) {
            ...
            // _publish_accel()
            auto & ins_state = m_ins_state[i];
           // auto & accel_state = m_accel_state[i];
            ins_state.accel.healthy = false;
            ins_state.accel.delta_velocity_valid = false;

            ins_state.gyro.healthy = false;
            ins_state.gyro.delta_angle_valid = false;
        }
        for (uint8_t i=0; i<_backend_count; i++) {
            _backends[i]->update();
        }
        ...
        for (uint8_t i = 0; i < m_max_instances; i++) {
           auto & ins_state = m_ins_state[i];
         //  auto & accel_state = m_accel_state[i];
           ins_state.accel.delta_velocity_acc.zero();
           ins_state.accel.delta_velocity_acc_dt = 0;
           ins_state.gyro.delta_angle_acc.zero();
        }
        ...
        for (uint8_t i=0; i<m_max_instances; i++) {
            auto & ins_state = m_ins_state[i];
           // auto const & accel_state = m_accel_state[i];
            if (ins_state.accel.healthy && ins_state.accel.error_count == 0) {
                have_zero_accel_error_count = true;
            }
           // auto const & gyro_state = m_gyro_state[i];
            if (ins_state.gyro.healthy && ins_state.gyro.error_count == 0) {
                have_zero_gyro_error_count = true;
            }
        }

        for (uint8_t i=0; i<m_max_instances; i++) {
            //auto & gyro_state = m_gyro_state[i];
            //auto & ins_state = m_ins_state[i];
            auto & gyro_state = m_ins_state[i].gyro;
            if (gyro_state.healthy && gyro_state.error_count != 0 && have_zero_gyro_error_count) {
                // we prefer not to use a gyro that has had errors
                gyro_state.healthy = false;
            }
            auto & accel_state = m_ins_state[i].accel;
            if ( accel_state.healthy && (accel_state.error_count != 0 && have_zero_accel_error_count)) {
                // we prefer not to use a accel that has had errors
               accel_state.healthy = false;
            }
        }
        ...
        for (uint8_t i=0; i<m_max_instances; i++) {
            auto & ins_state = m_ins_state[i];
            if (ins_state.gyro.healthy && ins_state.use) {
                _primary_gyro = i;
                break;
            }
        }
        for (uint8_t i=0; i<m_max_instances; i++) {
            auto & ins_state = m_ins_state[i];
            if (ins_state.accel.healthy && ins_state.use) {
                _primary_accel = i;
                break;
            }
        }
...
{
    auto const & gyro_state = m_ins_state[i].gyro;
    if (gyro_state.delta_angle_valid) {
        delta_angle = gyro_state.delta_angle;
        return true;
...
{
    if (m_ins_state[i].accel.delta_velocity_valid) {
        delta_velocity = m_ins_state[i].accel.delta_velocity;
        return true;
...
{
    if ( m_ins_state[i].accel.delta_velocity_valid ) {
        return m_ins_state[i].accel.delta_velocity_dt;
    }
...
void AP_InertialSensor::set_accel(uint8_t instance, const Vector3f &accel)
{
    ...
    if (instance < m_max_instances) {
        m_ins_state[instance].accel.current_value = accel;
        m_ins_state[instance].accel.healthy = true;
        if (_accel_count <= instance) {
            _accel_count = instance+1;
        }
        if (! m_ins_state[_primary_accel].accel.healthy) {
...
void AP_InertialSensor::set_gyro(uint8_t instance, const Vector3f &gyro)
{
    ...
    if (instance < m_max_instances) {
        auto & gyro_state = m_ins_state[instance].gyro;
...
{
    if (instance < m_max_instances) {
        auto & accel_state =m_ins_state[instance].accel;
        accel_state.delta_velocity = deltav;
        accel_state.delta_velocity_dt = deltavt;
...
void AP_InertialSensor::calc_vibration_and_clipping(uint8_t instance, const Vector3f &accel, float dt)
{
    ...
    if (fabsf(accel.x) > AP_INERTIAL_SENSOR_ACCEL_CLIP_THRESH_MSS ||
        ...
        ++ m_ins_state[instance].accel.clip_count;
    }
