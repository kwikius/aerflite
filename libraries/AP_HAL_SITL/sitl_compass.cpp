...
void SITL_State::_update_compass(float rollDeg, float pitchDeg, float yawDeg)
{
    ...
    for (uint8_t i=0; i<=mag_buffer_length-1; i++) {
        time_delta_mag = ( delayed_time_mag > buffer_mag[i].time )
            ?(delayed_time_mag - buffer_mag[i].time)
            :(buffer_mag[i].time - delayed_time_mag ); // find difference between delayed time and time stamp in buffer
