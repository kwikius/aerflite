...
void SITL_State::_update_flow(void)
{
    ...
    using rad_per_s = SITL::sitl_fdm::rad_per_s;
    gyro(rad_per_s{_sitl->state.rollRate}.numeric_value(), 
         rad_per_s{_sitl->state.pitchRate}.numeric_value(), 
         rad_per_s{_sitl->state.yawRate}.numeric_value());

    OpticalFlow::OpticalFlow_state state;

    // NED velocity vector in m/s
    Vector3f velocity(_sitl->state.speedN.numeric_value(),
                      _sitl->state.speedE.numeric_value(),
                      _sitl->state.speedD.numeric_value());

    // a rotation matrix following DCM conventions
    Matrix3f rotmat;
    rotmat.from_euler(
       quan::angle::rad{_sitl->state.rollDeg}.numeric_value(),
          quan::angle::rad{_sitl->state.pitchDeg}.numeric_value(),
             quan::angle::rad{_sitl->state.yawDeg}.numeric_value()
    );
