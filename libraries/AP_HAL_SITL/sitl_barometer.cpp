...
void SITL_State::_update_barometer(float altitude)
{
    ...
    // add delay
    uint32_t best_time_delta_baro = 200U; // initialise large time representing buffer entry closest to current time - delay.
    uint8_t best_index_baro = 0; // initialise number representing the index of the entry in buffer closest to delay.

    // storing data from sensor to buffer
    if (now >= ( last_store_time_baro + 10U) ) { // store data every 10 ms.
    ...
    delayed_time_baro = (now <= _sitl->baro_delay)? now : (now - _sitl->baro_delay); // get time corresponding to delay

    // find data corresponding to delayed time in buffer
    for (uint8_t i=0; i<=baro_buffer_length-1; i++) {
        time_delta_baro = (delayed_time_baro >= buffer_baro[i].time)
         ?(delayed_time_baro - buffer_baro[i].time) // find difference between delayed time and time stamp in buffer
         :( buffer_baro[i].time - delayed_time_baro);
    ...
    if (best_time_delta_baro < 200U) { // only output stored state if < 200 msec retrieval error
