    ...
    mav_socket(false)
{
    ...
    dcm.from_euler(
         quan::angle::rad{fdm.rollDeg}.numeric_value(), 
            quan::angle::rad{fdm.pitchDeg}.numeric_value(), 
               quan::angle::rad{fdm.yawDeg}.numeric_value()
     );
}
...
void Gimbal::update(void)
{
    ...
    vehicle_dcm.from_euler(
       quan::angle::rad{fdm.rollDeg}.numeric_value(), 
            quan::angle::rad{fdm.pitchDeg}.numeric_value(), 
               quan::angle::rad{fdm.yawDeg}.numeric_value()
     );

    Vector3f vehicle_gyro = Vector3f(
      rad_per_s{fdm.rollRate}.numeric_value(),
         rad_per_s{fdm.pitchRate}.numeric_value(),
            rad_per_s{fdm.yawRate}.numeric_value()
    );
    Vector3f vehicle_accel_body = Vector3f(
         fdm.xAccel.numeric_value(), 
            fdm.yAccel.numeric_value(), 
               fdm.zAccel.numeric_value()
    );
