...
#else
void SITL_State::m_update_gps(SITL::sitl_fdm const & fdm, bool have_lock)
#endif
{
    //Capture current position as basestation location for
    if (!_gps_has_basestation_position && have_lock ) {
      _gps_basestation_data.latitude = fdm.latitude.numeric_value();
//############check this is asl
#pragma message "check this is asl"
      _gps_basestation_data.longitude = fdm.longitude.numeric_value();
//########################
      _gps_basestation_data.altitude = fdm.altitude_asl.numeric_value();
      _gps_basestation_data.speedN = fdm.speedN.numeric_value();
      _gps_basestation_data.speedE = fdm.speedE.numeric_value();
      _gps_basestation_data.speedD = fdm.speedD.numeric_value();
      _gps_basestation_data.have_lock = true;
    ...
    d.latitude = fdm.latitude.numeric_value() + glitch_offsets.x;
    d.longitude = fdm.longitude.numeric_value() + glitch_offsets.y;
    d.altitude = fdm.altitude_asl.numeric_value() + glitch_offsets.z;
    ...
    d.speedN = fdm.speedN.numeric_value();
    d.speedE = fdm.speedE.numeric_value();
    d.speedD = fdm.speedD.numeric_value();
