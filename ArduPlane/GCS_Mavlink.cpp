...
//            }
        ...
        case MAV_CMD_DO_SET_HOME:
            ...
            if (is_equal(packet.param1,1.0f)) {
                ...
                int32_t const lat = static_cast<int32_t>(packet.param5 * 1.0e7f);
                int32_t const lng = static_cast<int32_t>(packet.param6 * 1.0e7f);
                int32_t const alt = static_cast<int32_t>(packet.param7 * 100.0f);

                Location new_home_loc {};
                new_home_loc.lat = lat;
                new_home_loc.lng = lng;
                new_home_loc.alt = alt;

                plane.ahrs.set_home(new_home_loc);
                plane.home_is_set = HOME_SET_NOT_LOCKED;
#if CONFIG_HAL_BOARD == HAL_BOARD_QUAN
                quan::three_d::vect<int32_t> const osd_home{lat,lng,alt};
                AP_OSD::enqueue::home_location(osd_home);
#endif
