...
{
public:
    AP_InertialSensor_Backend(AP_InertialSensor &imu, int16_t product_id);
...
protected:
    ...
    void _rotate_and_correct_accel(uint8_t instance, Vector3f &accel) const;
   // rotate accel vector, scale, offset and publish
    ...
    uint32_t _accel_raw_sample_rate(uint8_t instance) const {
        return _imu.m_ins_state[instance].accel.raw_sample_rate;
    }
    ...
    uint32_t _gyro_raw_sample_rate(uint8_t instance) const {
        return _imu.m_ins_state[instance].gyro.raw_sample_rate;
    }
